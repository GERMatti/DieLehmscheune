...
export const load: PageServerLoad = async ({ params, locals }) => {
 ...
 const participants = await workshopService.getAllRegistrationsFromWorkshop(Number(params.id));
 ...
 const schemaDefault = z.object({
  ...
  categoryname: z.enum([categoryNames[0], ...categoryNames.slice(1)]).default(workshop.categoryname),
  maxparticipants: z.number().positive().default(workshop.maxparticipants),
 });

 const form = await superValidate(zod(schemaDefault));

 return { participants, workshop, form, categoryNamesArray, message: null };
};
...
export const actions: Actions = {
 default: async ({ request, locals }) => {
  ...
  const workshopService = new WorkshopService(locals.dbconn);
  const workshop = await workshopService.getWorkshopById(Number(form.data.id));
  ...
  // Check for changes
  const hasChanges =
      form.data.title !== workshop.title ||
