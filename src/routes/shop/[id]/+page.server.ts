...
export const load: PageServerLoad = async ({ params, locals }) => {
  ...
  const workshop: Workshop | undefined = await workshopService.getWorkshopById(
    Number(params.id),
  );

  const price: number | unknown = await workshopService.getWorkshopPrice(
    workshop!.categoryid,
  );
  const remainingSlots: number | unknown = await workshopService
    .getRemainingAppointments(workshop!.workshopid);
